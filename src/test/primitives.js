...
import MeshStatic from "./meshStatic.js";
...
export var createMesh = function (gl, arr) {
    var mesh = new MeshStatic(gl);
    mesh.setVertices(arr.vertices);
    if (arr.faces) mesh.setFaces(arr.faces);
    if (arr.uv && arr.facesUV)
        mesh.initTexCoordsDataFromOBJData(arr.uv, arr.facesUV);
...
Primitives.createGrid = function (gl) {
    var mesh = createMesh(
        gl,
        createGridArray.apply(this, slice.call(arguments, 1))
    );
    if (gl) {
...
Primitives.createCube = function (gl) {
    return createMesh(
        gl,
        createCubeArray.apply(this, slice.call(arguments, 1))
    );
};
...
Primitives.createCylinder = function (gl) {
    return createMesh(
        gl,
        createCylinderArray.apply(this, slice.call(arguments, 1))
    );
};
...
Primitives.createTorus = function (gl) {
    return createMesh(
        gl,
        createTorusArray.apply(this, slice.call(arguments, 1))
    );
};
...
Primitives.createPlane = function (gl) {
    return createMesh(
        gl,
        createPlaneArray.apply(this, slice.call(arguments, 1))
    );
};

Primitives.createArrow = function (
    gl,
    thick = 0.02,
    height = 2.0,
    rConeT = 3.5,
...
Primitives.createCubeArrow = function (
    gl,
    thick = 0.02,
    height = 2.0,
    rConeT = 3.5,
    ...
    var cube = createMesh(null, createCubeArray(0.2));
    cube.getMatrix()[13] = height * 0.5;

    var arrow = {
        vertices: null,
        faces: null,
    };
    Remesh.mergeArrays([base, cube], arrow);
    return createMesh(gl, arrow);
};

Primitives.createLine2D = function (
    gl,
    lx = 0.0,
    ly = 0.0,
    ux = 0.0,
