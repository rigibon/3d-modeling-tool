...
class Camera {
  ...
  constructor(main, viewp, projectionMatrix, viewMatrix, newcamera) {
    this._main = main;

    this.newcamera = newcamera;

    this._mode = Enums.CameraMode.ORBIT; // SPHERICAL / PLANE
    this._projectionType = Enums.Projection.PERSPECTIVE; // ORTHOGRAPHIC
    ...
    this._fov = Math.min(145, 150); // vertical field of view
    ...
    // pivot stuffs
    this._usePivot = false; // if rotation is centered around the picked point
  ...
  updateProjection(canvas) {
    // if (this._projectionType === Enums.Projection.PERSPECTIVE) {
      mat4.perspective(this._proj, (45.0 * Math.PI) / 180.0, canvas.clientWidth / canvas.clientHeight, 0.0, 100.0);
      this._proj[10] = -1.0;
  ...
  computeWorldToScreenMatrix(mat) {
    mat4.mul(mat, mat4.mul(mat, this._viewport, this._proj), this._view);
    return mat;
  }
  ...
  project(vector) {
    var out = vec3.create(0.0, 0.0, 0.0);
