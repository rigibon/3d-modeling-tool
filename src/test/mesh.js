...
import OctreeCell from "./octree.js";
...
export class Mesh {
    ...
    static sortFunction(meshA, meshB) {
        ...
        if (!aTr && bTr) return -1;
        return (
            (meshB.getDepth() - meshA.getDepth()) * (aTr && bTr ? 1.0 : -1.0)
        );
    }
    ...
    updateGeometry(iFaces, iVerts, frontFaces = null) {
        // // console.log(frontFaces);
        this.updateFacesAabbAndNormal(iFaces, frontFaces);
    ...
    allocateArrays() {
        ...
        for (var i = 0; i < nbFaces; ++i) faceLeaf[i] = null;
    }
    ...
    initColorsAndMaterials() {
        ...
        if (
            !this._meshData._colorsRGB ||
            this._meshData._colorsRGB.length !== len
        ) {
            var cAr = (this._meshData._colorsRGB = new Float32Array(len));
            for (i = 0; i < len; ++i) cAr[i] = 1.0;
        }
        if (
            !this._meshData._materialsPBR ||
            this._meshData._materialsPBR.length !== len
        ) {
    ...
    initFaceRings() {
        ...
        for (i = 0; i < nbFaces; ++i) {
            ...
            countRing[fAr[id + 2]]++;
            var i4 = fAr[id + 3];
            if (i4 !== Utils.TRI_INDEX) countRing[i4]++;
        }
        ...
        var vrf = new Uint32Array(
            Utils.getMemory(4 * nbFaces * 6),
            0,
            nbFaces * 6
        );
        acc = 0;
        ...
        this._meshData._vertRingFace = new Uint32Array(
            vrf.subarray(0, nbFaces * 3 + acc)
        );
    }
    ...
    initVertexRings() {
        ...
        var vertRingVert = (this._meshData._vertRingVert = new Uint32Array(
            this.getNbEdges() * 2
        ));
        var vrfStartCount = this.getVerticesRingFaceStartCount();
        var vertRingFace = this.getVerticesRingFace();
        ...
        for (var i = 0, l = this.getNbVertices(); i < l; ++i) {
            ...
            vrvStart += vrvCount;
            if (vrfEnd - vrfStart !== vrvCount) vertOnEdge[i] = 1;
        }
    }
    ...
    expandsVertices(iVerts, nRing) {
        ...
        var acc = nbVerts;
        var nbVertices = this.getNbVertices();
        var iVertsExpanded = new Uint32Array(
        ...
        var i = 0;
        for (i = 0; i < nbVerts; ++i) vertTagFlags[iVertsExpanded[i]] = tagFlag;
        ...
        while (nRing) {
            ...
            for (i = iBegin; i < nbVerts; ++i) {
                ...
                for (var j = start; j < end; ++j) {
                    ...
                    if (vertTagFlags[id] === tagFlag) continue;
    ...
    getVerticesFromFaces(iFaces) {
        ...
        var fAr = this.getFaces();
        var acc = 0;
        var verts = new Uint32Array(
            Utils.getMemory(4 * iFaces.length * 4),
            0,
            iFaces.length * 4
        );
    ...
    updateFacesAabbAndNormal(iFaces, frontFaces = null) {
        var faceNormals = this.getFaceNormals();
        ...
        for (var i = 0; i < nbFaces; ++i) {
            ...
            var xmin =
                v1x < v2x ? (v1x < v3x ? v1x : v3x) : v2x < v3x ? v2x : v3x;
            var xmax =
                v1x > v2x ? (v1x > v3x ? v1x : v3x) : v2x > v3x ? v2x : v3x;
            var ymin =
                v1y < v2y ? (v1y < v3y ? v1y : v3y) : v2y < v3y ? v2y : v3y;
            var ymax =
                v1y > v2y ? (v1y > v3y ? v1y : v3y) : v2y > v3y ? v2y : v3y;
            var zmin =
                v1z < v2z ? (v1z < v3z ? v1z : v3z) : v2z < v3z ? v2z : v3z;
            var zmax =
                v1z > v2z ? (v1z > v3z ? v1z : v3z) : v2z > v3z ? v2z : v3z;
    ...
    expandsFaces(iFaces, nRing) {
        ...
        var fAr = this.getFaces();
        var acc = nbFaces;
        var iFacesExpanded = new Uint32Array(
            Utils.getMemory(4 * this.getNbFaces()),
            0,
            this.getNbFaces()
        );
        iFacesExpanded.set(iFaces);
        var i = 0;
        for (i = 0; i < nbFaces; ++i) ftf[iFacesExpanded[i]] = tagFlag;
        var iBegin = 0;
        while (nRing) {
            --nRing;
            for (i = iBegin; i < nbFaces; ++i) {
                ...
                for (var j = 0; j < 4; ++j) {
                    var idv = fAr[ind + j];
                    if (idv === Utils.TRI_INDEX) continue;
    ...
    initRenderTriangles() {
        if (this.hasUV())
            this._meshData._UVtrianglesABC = this.computeTrianglesFromFaces(
                this.getFacesTexCoord()
            );
        this._meshData._trianglesABC = this.computeTrianglesFromFaces(
            this.getFaces()
        );
    }
    ...
    getWireframe() {
        ...
        var useDrawArrays = false;
        if (useDrawArrays) {
            if (
                this._meshData._drawArraysWireframe &&
                this._meshData._drawArraysWireframe.length === nbEdges * 2
            ) {
                return this._meshData._drawArraysWireframe;
            }
            cdw = this._meshData._drawArraysWireframe = new Uint32Array(
                nbEdges * 2
            );
        } else {
            if (
    ...
    updateDuplicateGeometry(iVerts) {
        if (!this.isUsingTexCoords() || !this.hasUV()) return;
    ...
    initTexCoordsDataFromOBJData(uvAr, uvfArOrig) {
        ...
        var tArTemp = new Float32Array(
            Utils.getMemory(nbVertices * 4 * 2),
            0,
            nbVertices * 2
        );
        var dup = [];
        ...
        tAr.set(tArTemp);
        var startCount = (this._meshData._duplicateStartCount = new Uint32Array(
            nbVertices * 2
        ));
    ...
    updateDrawArrays(iFaces) {
        ...
        var facesToTris = this.hasOnlyTriangles()
            ? null
            : this.getFacesToTriangles();
        ...
        if (!cdv || cdv.length < nbTriangles * 9) {
            cdv = this._meshData._DAverticesXYZ = new Float32Array(
                nbTriangles * 9
            );
            cdn = this._meshData._DAnormalsXYZ = new Float32Array(
                nbTriangles * 9
            );
            cdc = this._meshData._DAcolorsRGB = new Float32Array(
                nbTriangles * 9
            );
            cdm = this._meshData._DAmaterialsPBR = new Float32Array(
                nbTriangles * 9
            );
        }
        ...
        for (var i = 0; i < nbFaces; ++i) {
            ...
            if (id4 === Utils.TRI_INDEX) continue;
            id4 *= 3;
        ...
        if (this.isUsingTexCoords()) this.updateDrawArraysTexCoord(iFaces);
    }
    ...
    updateDrawArraysTexCoord(iFaces) {
        ...
        if (!cdt || cdt.length !== nbTriangles * 6)
            cdt = this._meshData._DAtexCoordsST = new Float32Array(
                nbTriangles * 6
            );
        ...
        for (var i = 0; i < nbFaces; ++i) {
            ...
            if (id4uv === Utils.TRI_INDEX) continue;
            id4uv *= 3;
    ...
    // TRANSFORM DATA
    /////////////////
    updateCenter(modelPosition = null) {
        var box = this.getLocalBound();

        if (modelPosition) {
            vec3.set(
                this._transformData._center,
                modelPosition[0] + (box[0] + box[3]) * 0.5,
                modelPosition[1] + (box[1] + box[4]) * 0.5,
                modelPosition[2] + (box[2] + box[5]) * 0.5
            );
        } else {
            vec3.set(
                this._transformData._center,
    ...
    updateMatrices(camera) {
        mat3.normalFromMat4(
            this._transformData._lastComputedEN,
            this._transformData._editMatrix
        );
        mat4.mul(
            this._transformData._lastComputedMV,
            camera.getView(),
            this._transformData._matrix
        );
        mat3.normalFromMat4(
            this._transformData._lastComputedN,
            this._transformData._lastComputedMV
        );
        mat4.mul(
            this._transformData._lastComputedMVP,
            camera.getProjection(),
            this._transformData._lastComputedMV
        );
        var cen = this._transformData._center;
        var m = this._transformData._lastComputedMVP;
        this._transformData._lastComputedDepth =
            m[2] * cen[0] + m[6] * cen[1] + m[10] * cen[2] + m[14];
    }

    computeLocalRadius() {
        var box = this.getLocalBound();
        return (
            0.5 * vec3.dist([box[0], box[1], box[2]], [box[3], box[4], box[5]])
        );
    }

    normalizeSize() {
        var scale = Utils.SCALE / (2.0 * this.computeLocalRadius());
        mat4.scale(this._transformData._matrix, this._transformData._matrix, [
            scale,
            scale,
            scale,
        ]);
    ...
    intersectRay(vNear, eyeDir, collectLeaves) {
        var nbFaces = this.getNbFaces();
        var collectFaces = new Uint32Array(
            Utils.getMemory(nbFaces * 4),
            0,
            nbFaces
        );
        return this._meshData._octree.collectIntersectRay(
            vNear,
            eyeDir,
            collectFaces,
            collectLeaves ? this._meshData._leavesToUpdate : undefined
        );
    }
    ...
    intersectSphere(vert, radiusSquared, collectLeaves) {
        ...
        return this._meshData._octree.collectIntersectSphere(
            vert,
            radiusSquared,
            collectFaces,
            collectLeaves ? this._meshData._leavesToUpdate : undefined
     ...
     */
    updateOctree(iFaces) {
        if (iFaces) this.updateOctreeAdd(this.updateOctreeRemove(iFaces));
        else this.computeOctree();
    }
    ...
    updateOctreeRemove(iFaces) {
        ...
        var nbFaces = iFaces.length;
        var acc = 0;
        var facesToMove = new Uint32Array(
            Utils.getMemory(4 * nbFaces),
            0,
            nbFaces
        );
        ...
        for (var i = 0; i < nbFaces; ++i) {
            ...
            if (
                ...
                if (facesInLeaf.length > 0) {
                    // remove faces from octree cell
    ...
    balanceOctree() {
        ...
        for (var i = 0; i < nbLeaves; ++i) {
            ...
            if (leaf._flag === OctreeCell.FLAG) continue;

            if (leaf._iFaces.length === 0) leaf.pruneIfPossible();
            else if (
                leaf._iFaces.length > OctreeCell.MAX_FACES &&
                leaf._depth < OctreeCell.MAX_DEPTH
            )
                leaf.build(this);
    ...
    setShaderType(shaderName) {
        ...
        if (shaderName === Enums.Shader.UV && !hasUV) {
            if (this._renderData._shaderType !== Enums.Shader.UV) return;
    ...
    renderWireframe(main) {
        if (!this.isVisible()) return;
        Shader[Enums.Shader.WIREFRAME]
            .getOrCreate(this.getGL())
            .draw(this, main);
    }
    ...
    updateColorBuffer() {
        var colors = this.isUsingDrawArrays()
            ? this.getColorsDrawArrays()
            : this.getColors();
        this.getColorBuffer().update(colors, this.getRenderNbVertices() * 3);
    }

    updateMaterialBuffer() {
        var materials = this.isUsingDrawArrays()
            ? this.getMaterialsDrawArrays()
            : this.getMaterials();
        this.getMaterialBuffer().update(
            materials,
            this.getRenderNbVertices() * 3
        );
    }

    updateTexCoordBuffer() {
        if (this.isUsingTexCoords()) {
            var texCoords = this.isUsingDrawArrays()
                ? this.getTexCoordsDrawArrays()
                : this.getTexCoords();
            this.getTexCoordBuffer().update(
                texCoords,
                this.getRenderNbVertices() * 2
            );
        }
    }
    ...
    updateIndexBuffer() {
        if (!this.isUsingDrawArrays()) {
            var triangles = this.isUsingTexCoords()
                ? this.getTrianglesTexCoord()
                : this.getTriangles();
    ...
    updateWireframeBuffer() {
        if (this.getShowWireframe())
            this.getWireframeBuffer().update(
                this.getWireframe(),
                this.getNbEdges() * 2
            );
    }
    ...
    release() {
        if (this.getTexture0()) this.getGL().deleteTexture(this.getTexture0());
    ...
    copyData(mesh) {
        ...
        if (mesh.hasUV()) {
            this.initTexCoordsDataFromOBJData(
                mesh.getTexCoords(),
                mesh.getFacesTexCoord()
            );
        }
    ...
    optimize() {
        if (this.getNbFaces() === 0 || !Mesh.OPTIMIZE) return;
    ...
    optimizePostTransform() {
        ...
        for (i = nbUniqueVertices; i < nbVertices; ++i) {
            livesTriangles[i] =
                fringsStartCount[mapToUnique[i - nbUniqueVertices] * 2 + 1];
        }
        ...
        var deadEndStack = new Uint32Array(
            Utils.getMemory(4 * nbFaces * 4),
            0,
            nbFaces * 4
        );
        var deadEndCount = 0;
        ...
        while (fanningVertex >= 0) {
            ...
            var idRing =
                fanningVertex >= nbUniqueVertices
                    ? mapToUnique[fanningVertex - nbUniqueVertices]
                    : fanningVertex;
            ...
            for (i = start; i < end; ++i) {
                ...
                if (timeStamp - vTimeStamp[idv1] > cacheSize)
                    vTimeStamp[idv1] = timeStamp++;
                if (timeStamp - vTimeStamp[idv2] > cacheSize)
                    vTimeStamp[idv2] = timeStamp++;
                if (timeStamp - vTimeStamp[idv3] > cacheSize)
                    vTimeStamp[idv3] = timeStamp++;

                if (isQuad) {
                    deadEndStack[deadEndCount++] = idv4;
                    ringCandidates.push(idv4);
                    --livesTriangles[idv4];
                    if (timeStamp - vTimeStamp[idv4] > cacheSize)
                        vTimeStamp[idv4] = timeStamp++;
                }
            }
        ...
        fArUV = fArUVNew.subarray(0, nbFaces * 4);
        fAr = fArNew.subarray(0, nbFaces * 4);
    }
    ...
    optimizePreTransform() {
        ...
        if (nbUnusedVertex > 0) this.setNbVertices(acc);
