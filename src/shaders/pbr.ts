export var vPbr: string = `#version 300 es
...
export var fPbr: string = `#version 300 es
...
uniform int usingPBR;
uniform int flatShading;
uniform vec3 baseColorFactor;
uniform float metallicFactor;
uniform float roughnessFactor;
uniform float aoFactor;
uniform int usingBaseColorFactor;
uniform int usingMetallicFactor;
uniform int usingRoughnessFactor;
uniform int usingAoFactor;
in vec4 worldPos;
in vec3 normal;
...
void main(void) {
    if (usingPBR == 1) {
        ...
        // int nbLights = int(nLights);
        int nbLights = int(3.0);
        ...
        if (usingBaseColorFactor == 0) {
            albedo = pow(tex2DBiLinear(albedoMap, vec2(texCoords.s, texCoords.t)).rgb, vec3(gamma));
        } else {
            albedo = baseColorFactor;
        }

        if (usingRoughnessFactor == 0) {
            roughness = tex2DBiLinear(metallicMap, vec2(texCoords.s, texCoords.t)).g;
        } else {
            roughness = roughnessFactor;
        }

        if (usingAoFactor == 0) {
            ao = tex2DBiLinear(aoMap, vec2(texCoords.s, texCoords.t)).r;
        } else {
            ao = aoFactor;
        }

        if (usingMetallicFactor == 0) {
            fresnel = tex2DBiLinear(metallicMap, vec2(texCoords.s, texCoords.t)).b;
        } else {
            fresnel = metallicFactor;
        }

        vec3 normal = textureSize(normalMap, 0).x > 1 ? perturb_normal( vNorm, excidentVector, vec2(texCoords.s, texCoords.t)) : vNorm;
        // Fresnel f0 term
        vec3 f0 = vec3(0.04); 
        f0 = mix(f0, albedo, fresnel);
        for (int i=0 ; i<nbLights; i++){
            if(distance(pos,lights[i].position) <= MAX_DIST ){
                color = getLightColor(lights[i], pos);
                return;
            }
            vec3 incidentVector = normalize(lights[i].position-pos);
            ...
            vec3 radiance = vec3(lights[i].color) * getIntensityFromPosition(lights[i],pos);
        ...
        vec3 kD = 1.0 - kS;
        kD *= 1.0 - fresnel;
        vec3 irradiance = pow(texture(environmentMap, normal).rgb, vec3(4.0));
        vec3 ambientDiffuse = kD * irradiance * albedo;
        const float MAX_REFLECTION_LOD = 4.0;
        vec3 reflectionVector = reflect(-excidentVector, normal);
        vec3 prefilteredColor = pow(textureLod(prefilterMap, reflectionVector,  roughness * MAX_REFLECTION_LOD).rgb, vec3(4.0));    
        ...
        // Shadow computation
        vec3 lightDir = normalize(lights[nbLights-1].position-pos);
        float shadowFactor = 0.0;
        // Interpolation indicator gamma corrected
        float inter = pow(lights[nbLights-1].intensity / 200.0, 1.0/gamma);
        ...
        resultingColor = mix( resultingColor, vec3(1.0, 1.0, 1.0), PrintValue( (gl_FragCoord.xy - vec2(0.0, 5.0)) / vFontSize, 
            lights[nbLights-1].intensity / 200.0, 4.0, 10.0));*/
        ...
        color = vec4(tAmbientColor * 0.9 + tDiffuseColor * lambertian * 0.5, 1.0);
